---
# BASIC INFO
title: "TwistLens: A Docent-Informed Image Transformation to Create Previews That Prompt Anticipation and Interpretive Experiences Before Museum Visits"
author : "Thao Phuong Vu, <ins>Bokyung Lee</ins>"
Journal: "**Proceedings of the 2026 CHI Conference on Human Factors in Computing Systems (ACM CHI '26)**"

#Type: Journal_sci, Journal_int, Journal_dom, Conference_sci, Conference_int, Conference_dom
type: "Conference_sci"

# DATE INFO
date: 2026-03-01T23:59:58+09:00
publicationDate: 2026.04
homeYearTag: 2026


exactPeriod: 2026.04.13-17
cite_acm: "Thao Phuong Vu and Bokyung Lee. (2026). **TwistLens: A Docent-Informed Image Transformation to Create Previews That Prompt Anticipation and Interpretive Experiences Before Museum Visits**. _In Proceedings of the CHI Conference on Human Factors in Computing Systems (ACM CHI '26)_. Article 789, 1–21."
award: "Honorable Mention Award (Top 5%)"
doi: https://doi.org/10.1145/3772318.3790352


# PAGE PARAMETER
draft: false
link_PDF: "https://www.dropbox.com/scl/fi/f0so6wp98l2xnqaqnni5v/TwistLens_authors.pdf?rlkey=2pbfqkhlzqfq2wlxhip627uve&st=vdf4pjny&dl=0"

# links
links:
  - label : "PDF"
    link : "https://www.dropbox.com/scl/fi/f0so6wp98l2xnqaqnni5v/TwistLens_authors.pdf?rlkey=2pbfqkhlzqfq2wlxhip627uve&st=vdf4pjny&dl=0"
  - label : "Project Page"
    link : ""
  - label : "Source URL"
    link : "https://doi.org/10.1145/3772318.3790352"
---

TwistLens is a docent-informed, AI-supported image transformation system that creates spoiler-minimizing previews for pre-visit museum experiences. The method extracts informational cues from docent text through a structured taxonomy and applies transformation strategies (EchoLens and DecoyLens) to balance anticipation, interpretation, and representational coherence.
