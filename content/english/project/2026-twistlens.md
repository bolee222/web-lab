...
section_rows:
  ...
  - title: "Background: The Preview Dilemma"
    ...
    body_kr: >-
      박물관 경험은 관람객이 전시장에 들어가기 전부터 시작됩니다. 전시 웹사이트, 브로슈어, 소셜 미디어의 프리뷰는 도슨트 설명과 작품 이미지를 통해 관람 전 기대를 형성합니다. 하지만 이 두 가지 프리뷰 방식은 서로 다른 한계를 가집니다. 텍스트는 작품의 맥락, 상징, 해석을 풍부하게 전달하지만 아직 보지 못한 작품에 그 의미를 연결해 상상해야 합니다. 반대로 원본 이미지는 이해를 쉽게 만드는 시각적 기준점을 제공하지만, 작품을 너무 일찍 노출해 실제 관람에서의 발견과 놀라움을 약화시킬 수 있습니다. TwistLens는 바로 이 딜레마에서 출발합니다. 관람객이 작품을 이해할 수 있을 만큼의 단서를 제공하면서도, 원본을 마주하는 순간의 기대감은 어떻게 유지할 수 있을까요?
  ...
  - title: "Docent-Informed Preview Generation"
    body_en: >-
      TwistLens generates previews from a paired artwork image and docent-style description. First, the system analyzes the text to identify the most salient information category, such as material, style, composition, component description, regional knowledge, symbolic meaning, or curatorial intention. It also extracts cue phrases that connect the docent narrative to visible image regions. These phrases are grounded through segmentation so the system can transform the relevant parts of the image while keeping the surrounding scene stable. The result is not a generic blur or filter, but a semantically guided preview that controls what is revealed, what is withheld, and what is made curious.
    body_kr: >-
      TwistLens는 작품 이미지와 도슨트 설명을 함께 입력으로 받아 프리뷰를 생성합니다. 먼저 시스템은 도슨트 텍스트에서 가장 중요한 정보 유형을 찾습니다. 예를 들어 재료, 스타일, 구성, 특정 구성요소, 지역적 지식, 상징적 의미, 큐레이션 의도와 같은 범주가 여기에 포함됩니다. 이후 설명 속 단서 문구를 추출하고, 그 문구가 이미지의 어느 영역과 연결되는지 segmentation을 통해 찾습니다. 이렇게 식별된 영역만 선택적으로 변형하고 주변 장면은 유지하기 때문에, TwistLens의 프리뷰는 단순한 블러나 필터가 아닙니다. 무엇을 보여주고, 무엇을 숨기고, 어떤 부분에서 호기심을 만들지 의미 단위로 조절하는 프리뷰입니다.
  ...
  - title: "Co-Design with Art Enthusiasts"
    ...
    body_kr: >-
      변형 전략을 정교하게 만들기 위해 박물관을 자주 방문하고 도슨트 설명을 활용해 작품을 감상하는 21명의 참여자와 코디자인 연구를 수행했습니다. 연구에서는 8개의 정보 범주에 걸쳐 16개의 작품-설명 쌍을 준비했고, 각 작품마다 EchoLens 5개와 DecoyLens 5개의 프리뷰를 생성했습니다. 참여자들은 먼저 원본을 보지 않은 상태에서 변형 프리뷰를 해석하고, 이후 원본이 공개된 뒤 자신의 기대와 실제 작품 사이의 차이를 비교했습니다. 마지막으로 TwistLens 프로토타입을 직접 사용하며 개선 방향을 논의했습니다. 결과적으로 두 전략은 모든 정보에 똑같이 적용될 수 없었습니다. EchoLens는 전체적 시각 특성이나 상징적 서사를 전달할 때 선호되었고, DecoyLens는 특정 구성요소, 교육적 지식, 큐레이션 공간 구성처럼 주의를 집중시켜야 하는 정보에 더 효과적이었습니다.
    image: 
  ...
  - title: "Evaluation in Museum Preview Experiences"
    ...
    body_kr: >-
      이후 20명의 참여자를 대상으로 museum preview 경험에서 TwistLens를 평가했습니다. 참여자들은 두 가지 전시 브로슈어를 살펴보았습니다. 하나는 원본 이미지와 도슨트 텍스트를 제공하는 baseline 브로슈어였고, 다른 하나는 TwistLens가 생성한 변형 프리뷰와 도슨트 텍스트를 제공하는 브로슈어였습니다. 이후 참여자들은 두 개의 가상 전시를 관람하고 경험을 평가했습니다. 결과적으로 TwistLens는 관람 전 기대감과 호기심을 유의미하게 높였습니다. 실제 작품을 마주했을 때에는 관람 전 상상했던 이미지와 원본 사이의 차이가 더 큰 놀라움, 더 높은 관람 즐거움, 더 강한 스포일러 방지 효과로 이어졌습니다. 또한 TwistLens는 관람 전 창의적 사고를 촉진하고, 관람 후 정보 학습의 즐거움을 높이는 데에도 기여했습니다.
    image: 
